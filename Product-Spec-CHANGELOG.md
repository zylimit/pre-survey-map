...
### AREA 运营商区域面图层（第四类实体 · 全链路同 site 待遇）

**类型**：中度变更（新增实体表 + 树骨架扩展 + 导入/清洗/冲突/快照/导出/权限六链路各加一类）。

**触发**：用户提需求（优先级中）——导入运营商区域划分 KMZ（样例=Smart 区域），每个运营商目录下加 AREA 文件夹+面图层。

**决策（AskUserQuestion 两轮拍板）**：
- **数据模型 = 新表 `area`（第四类实体）**（否掉复用 lessor——lessor 是租户语义，混入会污染导出/自反契约）。
- **全链路同 site 待遇**：F13 清洗向导（面以质心做海里/基准国判定）+ NAME 去重冲突向导（同运营商内，带 operator 维度）+ F17 快照回滚 + KMZ 导出自反契约扩第四类（schema `#area`）+ 几何护栏只收面。
...
- **盖戳**：operator 强制写目标图层，源属性忽略（同 F20 模型）。

**Spec 改动**：F 功能表加 F23；新增「AREA 区域面图层（F23 · V1.x #51）」节。

**待补充**：SITE 点图层展示字段扩充——等 wenzhao 提供字段清单后单独立项。

**前置依赖**：需要用户提供 Smart 区域划分样例 KMZ 文件（解析契约 + 测试夹具用）。

---

### 用户与角色权限（登录页 + RBAC + 数据权限按图层文件夹）

**类型**：**重度变更 · 推翻 V1 Pinned 边界**（「V1 不做账号鉴权 / 身份识别=IP+UA+Session」作废；新增登录体系 + 4 张表 + 后端全接口鉴权 + 数据权限全链路过滤 + 管理界面；触碰 api/db/web，需重打镜像 + DB 迁移）。

**触发**：用户提需求——各运营商 PM 只该看自己运营商的站点，勘测执行者只该碰 SURVEY 目录；需用户管理 + 角色管理 + 按角色赋功能&数据权限。

**决策（AskUserQuestion 六问拍板）**：
- **登录形态 = 登录页（账号+密码）**（否掉无密码选身份/访问码——内网也要知道真人是谁）。
- **无权限文件夹 = 完全隐藏**（否掉灰显只读——树/地图/搜索/导出全不含，用户不知道自己看不到什么）。
- **功能权限 4 开关**：import / export / edit_delete / danger（清除基线+恢复点），管理界面（用户/角色/审计/备份）仅 admin、不入权限模型。
- **账号体系 = admin 内置 + 界面建号**：首次部署内置 `admin`/`admin123` 首登强制改密；忘密由 admin 重置（V1 无邮件，不做自助找回）。
- **权限粒度 = 树任意文件夹节点，子级继承**：`执行者` 拿三家 SURVEY 的配置要求粒度细到 运营商→类别 层。**查看=编辑同权**，不设只读态。
- **并发 = 不管，后写赢**：明确取舍非漏做；冲突靠审计追责。
- **隐藏入口收编**：F19 审计 3×ESC / F21 备份 3×B 废除，搬入 [⚙ 管理] Modal（仅 admin）。
- **仅内置 admin 角色**：Globe PM / Smart PM / DITO PM / 执行者 全由 admin 手建，不做预置。

**变更**：
- **新增 F22**：登录页（未登录全跳登录页，API 未认证 401）→ 用户归属角色 → 角色 = 功能权限 × 数据权限（文件夹节点集合，子级继承，后端统一 SQL 过滤，不靠前端藏）；导出 KMZ 只含可见数据。
- **新表 4 张**：`app_user` / `app_role`（perms JSONB + is_admin）/ `app_role_scope`（scope_node 如 `site:Globe:SURVEY`）/ `auth_session`（DB token 可吊销，7 天滑动过期——不用 JWT，admin 禁用/重置须立即生效）。
- **密码**：bcrypt 哈希，最小 8 位，连错 5 次锁 10 分钟；禁用/重置 → 该用户全部 token 立即吊销。
- **审计联动（F19 修订）**：身份识别叠加真实 `username`（表加列）；新增 5 类事件（`login`/`login_failed`/`logout`/`user_manage`/`role_manage`），15 → 20 类。
- **Toolbar 新增 [⚙ 管理] 按钮**（仅 admin 渲染）：用户管理 / 角色管理 / 审计日志（F19 Modal 搬入）/ 备份恢复（F21 Modal 搬入）。
- **UI**：登录页（全屏）+ Toolbar 右端当前用户名 + [登出]。

**冲突检测**：
- **vs Pinned 红线**：「V1 不做账号鉴权」「身份识别=IP+UA+Session」**作废**，progress.md Pinned 待同步更新；「V1 不做多人协同」保留——#50 只做多账号登录，并发冲突仍不做。
- **vs F19/F21 隐藏入口**：3×ESC / 3×B + mangosv5 硬编码密码废除；`audit_log` 后端约束（无 DELETE/PATCH、永久保留、元审计）不变。
- **vs F20 盖戳模型**：无冲突——数据权限是读取/操作过滤层，盖戳写库逻辑不动；无权限文件夹的 [导入图层] 按钮随文件夹一起隐藏。
- **vs F17 恢复点 / #49 删除撤销**：归 `danger` / `edit_delete` 功能权限控制，机制本身不动。
- **vs 已部署 v1.0.6**：升级需 DB 迁移（4 张新表 + `audit_log` 加 `username` 列）+ admin 内置账号种子；老库无用户体系，迁移后首次访问弹登录页。

**Spec 改动**：F 功能表加 F22 + F19/F21 行收编注记；新增「用户与角色权限（F22 · V1.x #50）」节；F19 节（动机/身份识别/隐藏入口/枚举数）修订；#48 权限条修订；V1 边界声明「认证」行、V2 候选「认证」条标注已实现；Toolbar 表加 [⚙ 管理]。

**待办（进入 dev-planner / dev-builder）**：本条仅完成 Spec + CHANGELOG；登录页、4 张表迁移、auth/user/role 后端接口、鉴权中间件、数据权限 SQL 过滤、管理 Modal、审计联动、隐藏入口拆除、回归测试均未实现。
