...
| 🗑️ 删除历史 | 打开**「删除历史」面板（V1.x #49）**：列出最近 200 个 site 删除批次（时间 · 删 N 条 · 所属图层 · 站点名摘要），每批 [撤销] 把该批被删行再插回（主键被重新占用则跳过、报实际恢复数）；撤销后该批标记「已撤销」移出列表。轻量 `site_delete_undo` 兜底，**非** F17 全表快照。**【V1.x #50】需「编辑删除」功能权限** |
| ⚙ 管理 | **【V1.x #50 新增】** 仅 **admin** 可见。打开管理 Modal，四块：**用户管理**（建号/重置密码/禁用）/ **角色管理**（角色 CRUD + 权限勾选）/ **审计日志**（收编 F19，废 3×ESC 隐藏入口）/ **备份恢复**（收编 F21，废 3×B 隐藏入口）。非 admin 用户不渲染此按钮 |
...
- **版本号显示（V1.x #38）**：折叠条右侧（DB 状态圆点旁）显示 `v1.0.0 · 构建 2026-06-02 14:30` —— 软件**语义版本**（major.minor.patch；本次内网首发定 **v1.0.0**，修 bug→patch、加功能→minor、大重构→major）+ **构建时间**。让工程师一眼知道当前版本、是否刚部署新版（构建时间变 = 新版已生效，正解「不知道更没更新」的痛点）。
  - **单一真源** = `web/package.json` 的 `version` 字段；构建时间由 **Vite 构建时注入**（`define: __APP_VERSION__ / __BUILD_TIME__`），不手填、不漂移。
  - **与部署版本打通**：该版本号同时用作 docker 镜像 tag / 部署包前缀（`update-deploy.sh <版本>`），界面显示 = 镜像 = 部署包，三处一致。
...
| 🎨 | **样式** | **[查看]（V1.x #40，眼睛 · 不带导入）** | 有（控制该状态子集显隐） | 图层下按 site_status 分色的图例（positive 绿 / negative 红 / undermine 黄 + 灰 Other，#33 起红黄对调）；**固定挂载、0 要素也显示**（#33）；显示该状态要素数；**[查看] 打开列表框并按该 site_status 筛选**（#40，相当于基于状态的筛选）；要素在列表框 |
...
│  │  └─ 🔺 IBS              [导入][查看]
│  ├─ 📁 规划                        ← 文件夹（类别）
│  │  ├─ 🔺 Macro NP         [导入][查看]   ← 含辐射圈(半径全局可配)
│  │  └─ 🔺 Micro NP         [导入][查看]   ← 含辐射圈(半径全局可配)
│  └─ 📁 勘测                        ← 文件夹（类别）
│     ├─ 🔺 Macro-ongoing    [导入][查看]
│     └─ 🔺 Micro-ongoing    [导入][查看]
├─ 📁 Smart   … （同 Globe 子结构）
...
- **浮动窗口交互（V1.x #30）**：列表框是一个**可拖、可缩的固定尺寸浮动窗口**，不再随行数变高：
  ...
  - **可缩放（V1.x #43 八方向）**：四边（左/右/上/下）+ 四角共 8 个拖拽手柄，**全方向拉伸**（不再仅右下角）；拖含左/上方向时锚点在对侧、同步改 left/top + size；最小 480×320、最大视口 − 32px，clamp / rAF 节流 / 尺寸持久化不变
...
#### 要素增删改与勾选导出（site · V1.x #48）

> 兑现上文「编辑能力是下一步独立需求」。把「查看图层要素」列表框从**只读**升级为**可操作**。**本期仅 site 图层**（最复杂、9 列 + 盖戳 + 坐标），road/lessor 沿用只读，待 site 验证交互与回滚链路后再复制。

- **行前多选框（checkbox）**：列表每行最前面加一列复选框；表头处放**全选/反选**框（仅选当前筛选后的可见子集，与筛选框口径一致）。多选状态驱动下方批量删除 / 勾选导出。
- **单行编辑（一次一行）**：每行末尾（或选中行）出 [✏️ 编辑] → 弹**编辑表单**（小 modal/inline 面板）。
  - **可改字段**：业务属性（`PROJECT` / `SITE STATUS` 等）+ **坐标 `LATI` / `LONGI`**。改坐标后**后端同步重算 `geom`**（PostGIS Point，从 lati/longi 重建），保证地图点与属性一致。
...
- **批量删除（勾选多行）**：勾选 N 行 → [🗑️ 删除选中] → 确认 modal（列出将删数量）→ 后端批量删除。
  ...
  - **撤销 = 持久「删除历史」面板（V1.x #49）**：删除返回 `undo_id`，但撤销入口**不是瞬时 toast**，而是**工具栏新增 [🗑️ 删除历史] 按钮**（与 [🕘 恢复点] 并列）打开的持久面板——列出最近删除批次，每批可**回头逐批撤销**（这才配得上保留 200 批）。撤销 = 把该批次行**再插回** site，主键已被重新占用则跳过并报实际恢复数。
  - **面板每批显示**：`时间 · 删 N 条 · 所属图层 · 站点名摘要`。**撤销后**该批标记「已撤销」并移出可撤列表（防重复撤销）。
...
- **列宽可拖拽调整（类 Excel · 纯前端，三个 kind 通用）**：表头每列右边界放拖拽手柄，**拖动手动调该列宽度**（像 Excel 拖列边）。
  ...
  - **持久化**：手动列宽存 `localStorage`（按 kind + 列 key，如 `presurvey.lfl.col.site.lati`），下次开窗恢复；未调过的列不写、保持自适应。
  - **双击分隔线 = 自适应**：双击列分隔手柄 → 该列按内容最宽值自动收放（Excel 双击自适应惯例）；可作为"清掉手动宽、还原自动"的快捷。
  - **最小列宽守卫**：每列有最小宽（如 48px）防拖没；拖动走 `requestAnimationFrame` 节流（与 #30/#43 窗口拖缩一致，防大数据掉帧）。
...
- **后端新增接口（site，本期）**：
  ...
  - `GET /api/sites/delete-history`（#49）— 列最近删除批次给「删除历史」面板：每批 `undo_id / deleted_at / 条数 / 图层(operator·category·type 摘要) / 站点名摘要 / undone`，按时间倒序。
  - `POST /api/sites/undo-delete/{undo_id}`（#49）— 把该批次被删行**再插回** site；主键已被重新占用则 `ON CONFLICT DO NOTHING` 跳过；该批次标记 `undone`；返回实际恢复数。
  - 勾选导出复用现有导出链路（按 id 子集，非按几何选区）；若现有 `export_selection` 仅吃 polygon，则加一条按主键列表导出的通道。
- **新建表 `site_delete_undo`（#49）**：镜像 site 列 + `undo_id`（批次）+ `deleted_at` + `undone`（已撤销标记）；环形保留最近 200 批。轻量、与库规模无关，**取代 #48 复用 F17 全表快照的方案**。
...
- **权限**：~~V1 不做登录/权限（与「清除基线」一致），仅靠确认 modal 防误删~~ **【V1.x #50 起】编辑/删除需 `edit_delete` 功能权限 + 该图层数据权限**（见 F22）；删除仍有「删除历史」撤销兜底（#49），风险可控。
- **范围边界（本期不做）**：① road/lessor 增删改（沿用只读）；② 改主键 / 改盖戳三列（跨层移动）；③ 新建要素（要素仍只能从「导入图层」进，保留盖戳 + 几何护栏 + 去重链路，列表框不开"凭空建点"）。

#### 样式与渲染（形状 × 颜色）

- **形状来自图层（站型 type）**，**颜色来自状态（site_status）**，二者叠加 = 地图上的最终图标（如"绿色三角""黄色五角星"）。**（V1.x #44：颜色按 category 分叉——仅勘测 × status 色；存量 × 橙 `#f97316` 实心；规划 × 紫 `#a855f7` 半透明填充 55% + 紫描边；形状仍按 type；辐射圈跟随变紫）**
...
- **F13 清洗向导**：面要素走清洗——在海里/不在基准国以 ST_Centroid 质心判定；**「坐标写反/漏小数点」规则不适用于面**（面无 lati/longi 列，写反检测是点列算术规则；异常坐标面由质心判定兜底）（#51 review 裁决修订）
...
**动机**：V1 是单工程师工具，Pinned 边界写明"不做账号鉴权"。实际使用扩展出多角色场景——各运营商 PM 只该看自己运营商的站点，勘测执行者只该碰 SURVEY（勘测）目录。**#50 正式推翻 V1「认证/鉴权不做」的边界**，引入轻量 RBAC。设计前提：仍是内网工具、人数个位数、无邮件能力——不引入 OAuth/SSO/邮箱找回这类重设施。

#### 登录（账号 + 密码）

- 访问任何页面 → 未登录一律跳**登录页**（全屏页，仅 账号 / 密码 / [登录]）；后端所有 API（除 `/api/auth/login` 与健康检查）**未认证一律 401**。
- 登录成功 → 后端签发 **session token**（DB 存 `auth_session` 表，可吊销；非 JWT——admin 禁用用户/重置密码须立即生效，JWT 吊销不了），前端存 localStorage，请求带 `Authorization: Bearer <token>`。**7 天滑动过期**（有活动自动续，7 天无操作踢出）。
- 登出 → 吊销当前 token。管理员禁用用户 / 重置密码 → 该用户**所有 token 立即吊销**（下次请求 401 弹回登录页）。
- 密码：**bcrypt 哈希存储**，最小 8 位；登录连续失败 5 次锁 10 分钟（防内网脚本爆破，够用即止）。
- **初始账号**：首次部署内置 `admin` / 默认密码 `admin123`（部署文档写明），**首次登录强制改密**（不改完进不了主界面）。
- 主界面 Toolbar 右端显示当前用户名 + [登出]；**登录后各主流程（导入/导出/框选/恢复点…）完全不变**。

#### 数据模型（新增 4 表）

| 表 | 核心列 | 说明 |
|----|--------|------|
| `app_user` | `username`(唯一) / `password_hash` / `role_id`(FK) / `disabled` / `must_change_password` / `created_at` | 用户归属唯一角色 |
| `app_role` | `name`(唯一) / `is_admin` / `perms JSONB`（功能权限 4 开关）/ `created_at` | `is_admin=true` 仅内置 admin 角色，**不可删、不可改权限** |
| `app_role_scope` | `role_id` / `scope_node`(如 `site` / `site:Globe` / `site:Globe:SURVEY` / `road` / `lessor`) | 数据权限 = 文件夹节点集合，子级继承 |
| `auth_session` | `token`(主键) / `user_id` / `expires_at` / `created_at` | 吊销 = 删行 |

#### 权限模型（两层，交集生效）

**功能权限（角色级 4 开关，存 `app_role.perms`）**：

| 开关 | 控制的功能 |
|------|-----------|
| `import` | 图层 [导入图层] |
| `export` | 导出整库 / 导出选区 / 勾选导出 KMZ |
| `edit_delete` | site 单行编辑 / 批量删除 / 删除历史撤销 |
| `danger` | 清除基线 / 恢复点（建点/回滚/删除） |

**数据权限（文件夹节点，子级继承）**：
- 可勾节点 = 树的**任意文件夹节点**：`SITE` 根 / 各运营商（Globe/Smart/Dito）/ 运营商下类别（EXISTING/PLANNED/SURVEY）/ `Road` / `Lessor`。勾父节点 → 全部后代自动有权。
- **查看 = 编辑同权**：能看即可编辑，不设只读态（用户拍板，省一套权限维度）。
- **无权限 = 完全隐藏**：树上不渲染该文件夹、地图不渲染其要素、列表框/全局搜索/属性面板/导出（整库/选区/勾选）一律不含——**后端统一过滤**（按可见 scope 换算 operator/category 子集做 SQL WHERE），不靠前端藏。
- 导出 KMZ 只含可见数据；`np-radius-rings` 同理按可见 NP 点生成。
- **管理界面（用户/角色/审计/备份）不入权限模型**——仅 `is_admin` 可见可进，不给普通角色开这个口。

#### 管理界面（Toolbar [⚙] · 仅 admin）

- **用户管理**：用户列表（用户名/角色/状态/创建时间）→ [新建用户]（用户名 + 选角色 + 初始密码，初始密码登录后强制改）/ [重置密码]（admin 直接设新密码）/ [禁用·启用]。**admin 用户不可禁用、不可删**；系统至少保留一个 admin 账号。
- **角色管理**：角色列表 → [新建角色]（名称 + 4 个功能权限开关 + 文件夹节点勾选树，三态 checkbox 体现子级继承）/ [编辑] / [删除]（**有用户挂载的角色不可删**，先迁用户）。内置 `admin` 角色只读展示。
- **审计日志**：F19 原 Modal 原样搬入（3×ESC 隐藏入口废除）。
- **备份恢复**：F21 原 Modal 原样搬入（3×B 隐藏入口废除）。
- 典型配置（admin 手建，图上示例）：角色 `Globe PM` 勾 `site:Globe` + import/export/edit_delete；`Smart PM` / `DITO PM` 同理；角色 `执行者` 勾 `site:Globe:SURVEY` + `site:Smart:SURVEY` + `site:Dito:SURVEY`。**仅内置 admin，其余角色全手建，不做预置**。

#### 审计联动（F19 修订）

- **身份识别升级**：原 IP+UA+SessionID 三件套保留，**叠加真实 `username`**（审计表加 `username` 列，登录后所有记录写真人）。
- **新增 5 类审计事件**（F19 由 15 类 → 20 类）：`login`（成功）/ `login_failed`（失败，记尝试账号）/ `logout` / `user_manage`（建号/重置/禁用，details 记目标用户+动作）/ `role_manage`（建/改/删角色，details 记角色名+权限快照）。
- 审计/备份界面虽收编，但 `audit_log` 表后端约束不变（无 DELETE/PATCH 端点、永久保留、元审计照旧）。

#### 并发策略（用户拍板）

多人同时操作**不处理**——后写赢（编辑/删除/导入以 commit 时间为准，无锁无版本校验）。冲突概率低（各管各的运营商目录），真出冲突靠审计日志追责。这是明确取舍，不是漏做。

#### 边界（本期不做）

- 无注册、无忘记密码自助找回（忘密找 admin 重置）、无密码复杂度策略（仅最小 8 位）、无 2FA
- 无只读角色（查看=编辑同权）、无字段级权限
- 多设备同时登录不限制
...
**动机**：~~V1 不做登录~~（V1.x #50 起已引入登录+RBAC，见 F22），关键操作（导入/导出/清基线/回滚/记录点变更）必须有痕迹，方便追责和复盘。

#### 身份识别 ~~= 方案 E（仅技术信息，不识别真人）~~ **【V1.x #50 升级】= 真实 username + 技术信息**
...
| `username`（#50 新增） | 登录态 token 反查 `app_user` | **真实操作人**；未登录请求（V1.x #50 起仅剩 login/健康检查）记空 |

**明确不记录的（V1 受限于浏览器安全）**：Windows 域账号、机器名、MAC 地址。~~登录用户~~（#50 起已由 username 列覆盖）。

#### 操作枚举（~~12 类~~ → #48/#49 扩至 15 类 → **#50 再增 5 类 = 20 类**：`login` / `login_failed` / `logout` / `user_manage` / `role_manage`，见 F22「审计联动」）
...
#### 隐藏入口（F19 · 雷 C）~~【V1.x #50 废除】~~

> **#50 起**：审计日志入口收编进 Toolbar [⚙ 管理] Modal（仅 admin），**3×ESC + mangosv5 隐藏入口废除**。以下为历史记录保留备查。
...
| 类别 | V1 不做的事 |
|------|------------|
| 认证 | ~~用户账号 / 鉴权~~（**V1.x #50 起已做**：登录页 + RBAC，见 F22） |
...
V1 不做、未来视情况引入。每项随时间评估优先级。**【2026-08-18 #6 评估结论】**：#50 RBAC 落地后重排——多账号已能用，V2 剩余价值排序如下。

### 工作流类
- **基线 / 个人空间双工作区**（Git-like staging area）——**评估：低优先级，搁置**
  - 工程师所有导入/编辑操作只影响个人空间副本，与基线对比 diff，确认后批量同步（commit）到基线
  - 动机：① 避免频繁写后端库；② 提交前可视化对比变更
  - **#6 评估**：#50 后各 PM 已有数据权限隔离（各管各的目录），"误伤别人数据"的动机大幅减弱；F17 恢复点 + #49 撤销删除已覆盖"操作可回退"；双工作区引入的存储形态/同步冲突复杂度与剩余收益不成比例。除非出现"导入前必须预览 diff"的真实诉求，不动。
  - 设计待定：个人空间存储形态（前端 IndexedDB vs 后端 workspace 表）、身份识别方式、同步冲突解决、派工导出从哪个空间
- **多人协同 + 冲突解决**：操作日志、并发锁、回滚——**评估：不做**。#50 已拍板"后写赢 + 审计追责"，各 PM 数据域天然不重叠，冲突面≈0。
- ~~**认证 / 鉴权 / 账号体系**~~（**V1.x #50 已实现**，见 F22）
- **历史版本**：基线数据的时间快照——**评估：低优先级**。F17 恢复点（10 点环形）+ F21 定时备份（12h/30 天）已构成事实上的历史快照链，可人工回滚到任一点；专门的时间轴浏览 UI 收益低。
- **工作空间（一空间 = 一国家）**（V1.x #15 用户提出）——**评估：中优先级，等真实信号**。当前只跑菲律宾一个国家，无多国家并行诉求；出现第二个国家项目时再做（schema 隔离或 tenant_id，改动面大，不提前投资）。

### AI 能力类
- **列名智能识别**：自动映射 `Site_ID` → `SITE ID`，缓解「场景 3」问题——**评估：低优先级**。现有解析失败红字提示 + 候选列名列表已把成本降到人工 10 秒；内网数据格式相对固定。
- **坐标异常 AI 检测**：识别 `7177416` 这类漏小数点的脏数据——**评估：低优先级**。F13 清洗向导四类规则（写反/漏小数点/海里/非基准国）已覆盖实际遇到的全部脏数据形态。
- **名称模糊去重**：`"Huawei Technologies"` vs `"HUAWEI TECHNOLOGIES PHILIPPINES INC"` 之类公司名变体——**评估：中优先级，Lessor 数据多了再启动**。当前库空，无真实样本；等 Lessor 积累到出现重名变体时再做（那时有真实数据可验证效果）。
- **冲突智能合并建议**：多人协同时帮工程师快速决策——**评估：不做**（多人冲突已拍板后写赢，无合并场景）。

### UI 增强类
- **单要素删除（软删除 + 回收站）**（V1.x #24 · F20 时明确推迟到 V2）——**评估：基本被 #48/#49 消化**。勾选批量删除 + 删除历史撤销（200 批）已覆盖"删错了能找回"；软删除回收站的增量价值只剩"长期保留已删记录"，当前无诉求。**关闭此项**（如未来要"已删要素长期可查"，靠审计 delete_site 记录已够追溯）。
- 树节点右键菜单（删除、导出此要素等）——**评估：低优先级**。现有入口（图层按钮/列表框）已可达全部操作，右键是效率优化非能力缺失。
- 属性面板分 tabs（基本 / 联系人 / 技术 / 租赁）——**评估：低优先级**。属性量不大，单面板滚动可看全。
- 点聚合（OpenLayers Cluster）——**评估：中优先级，等性能信号**。当前 7300 点渲染无卡顿报告；数据涨到 2-3 万点出现卡顿时启动。
- 地图上直接编辑要素（拖动、改属性、删除）——**评估：低优先级**。列表框编辑已覆盖改属性/坐标；拖点改坐标是高频误操作源，不建议开。
- 邮件自动发送派工 KMZ——**评估：中优先级，最有真实价值的候选项**。派工"导出 KMZ → 手动发邮件"是当前流程里唯一的人工断点；内网需可发信的 SMTP 中继（部署环境确认项）。有真实派工频率数据后启动。
