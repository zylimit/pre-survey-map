...
| 🗑️ 删除历史 | 打开**「删除历史」面板（V1.x #49）**：列出最近 200 个 site 删除批次（时间 · 删 N 条 · 所属图层 · 站点名摘要），每批 [撤销] 把该批被删行再插回（主键被重新占用则跳过、报实际恢复数）；撤销后该批标记「已撤销」移出列表。轻量 `site_delete_undo` 兜底，**非** F17 全表快照 |
...
- **权限**：V1 不做登录/权限（与「清除基线」一致），仅靠确认 modal 防误删；删除有**「删除历史」撤销**兜底（#49），风险可控。
